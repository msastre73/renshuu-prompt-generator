import { Stack, Checkbox, TextInput, Box, Text, Flex, Button, Select, Anchor } from '@mantine/core';
...
export function PromptConfigForm({ onSubmit, isLoading, isPromptError }: PromptConfigFormProps) {
    const lastPromptConfig = useSelector((state: RootState) => state.user.lastPromptConfig);

    const form = useForm<PromptConfig>({
        initialValues: {
            includeFurigana: lastPromptConfig?.includeFurigana ?? true,
            includeSuperscript: lastPromptConfig?.includeSuperscript ?? true,
            conversationTopic: '',
            selectedSchedulesIds: [],
            selectedWordsStatus: lastPromptConfig?.selectedWordsStatus ?? ['studied'],
            japaneseLevel: lastPromptConfig?.japaneseLevel ?? '',
        },
        validate: {
            japaneseLevel: (value) => {
                if (!value) {
    ...
    return (
        <Stack>
            {lastPromptConfig &&
                <Text size="sm" ta="center">Or you can <Anchor onClick={() => {
                   // Scroll to the bottom
                   window.scrollTo({
                    top: document.body.scrollHeight,
                    behavior: 'smooth'
                   });
                    onSubmit(lastPromptConfig);
                }}>repeat your last prompt</Anchor></Text>
            }
